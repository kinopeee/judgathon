...
- Extraction language neutrality: `{{output_language}}` controls ONLY the language in which
  `description` is written. Which facts you select, their `kind`, their `sources`, and their
  `criterion_hints` MUST NOT depend on the output language. Extract the same set of facts —
  in particular the same `observation` items from the frames — regardless of whether the
  output language matches the pitch language. Never omit, add, or reclassify a fact because
